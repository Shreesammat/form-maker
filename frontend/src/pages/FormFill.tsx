...
export default function FormFillPage() {
  ...
  const validateForm = () => {
    ...
    form.questions.forEach((question) => {
      ...
      if (question.type === "comprehension" && question.extraData?.subQuestions) {
        const subQuestions = question.extraData.subQuestions;
        subQuestions.forEach((subQ: any) => {
          ...
          if (subQ.required && (!subAnswer || (typeof subAnswer === 'string' && subAnswer.trim() === ""))) {
            newErrors[`${question._id}-${subQ.id}`] = "This field is required";
          }
        });
      } else {
  ...
  const renderClozeQuestion = (question: Question, index: number) => {
    ...
    const blanks = question.extraData?.blanks || [];
    const questionId = question._id || "";
    const hasError = !!errors[questionId];
                  ...
                  <span key={partIndex}>
                    ...
                    {blankIndices[partIndex] !== undefined && (
                      <Input
                        ...
                        value={answers[questionId]?.[blankIndices[partIndex] - 1] || ""}
                        onChange={(e) => {
                  ...
                  return (
                    <div key={subQ.id} className="p-3 border rounded-lg">
                      ...
                      {subQ.type === "multiple_choice" && subQ.options && (
                        <RadioGroup
                          value={answers[questionId]?.[subQ.id] || ""}
                          onValueChange={(value) => {
                            const currentAnswers = answers[questionId] || {};
                          ...
                          {subQ.options.map((option: string, optIndex: number) => (
                            <div key={optIndex} className="flex items-center space-x-2">
                              <input
                                ...
                                checked={answers[questionId]?.[subQ.id]?.includes(option) || false}
                                onChange={(e) => {
                                  const currentAnswers = answers[questionId] || {};
                                  const currentSelections = currentAnswers[subQ.id] || [];
  ...
  const requiredQuestions = form.questions.length; // Simplified for now
  const answeredRequired = Object.keys(answers).length;
    ...
    <div className="container mx-auto py-8 max-w-4xl">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between text-sm text-gray-600 mb-2">
          ...
          <span>{answeredRequired}/{requiredQuestions}</span>
        </div>
        <div className="w-full bg-gray-200 rounded-full h-2">
          <div
            className="bg-blue-500 h-2 rounded-full transition-all duration-300"
            style={{
              width: `${(answeredRequired / requiredQuestions) * 100}%`,
            }}
          ></div>
        </div>
      </div>
        ...
        <CardContent className="pt-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <Badge variant={answeredRequired === requiredQuestions ? "default" : "outline"}>
                {answeredRequired}/{requiredQuestions} completed
              </Badge>
            </div>
